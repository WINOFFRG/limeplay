...
export type useTimelineStatesProps = {
  /**
...
export function useTimelineStates({
  ...
  const onTimeUpdate = () => {
    ...
    if (readyState < MediaReadyState.HAVE_METADATA) {
      return;
    }
...
export function useTimeline() {
  ...
  const seek = useCallback(
    (time: number) => {
      if (!(mediaRef.current && Number.isFinite(duration))) {
        return;
      }
  ...
  const setHoveringTime = useCallback(
    (time: number) => {
      if (!Number.isFinite(store.getState().duration)) {
        return;
      }
  ...
  const processBufferedRanges = useCallback(
    (
      ...
      if (variant === "combined") {
        ...
        if (combinedBuffered.start !== Number.POSITIVE_INFINITY) {
          normalizedBuffered = [
            {
