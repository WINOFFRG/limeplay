...
| --- | --- | --- |
| Opinionated blocks and `PlaybackSourceController` | You are building reusable player blocks. | Normalizes `source`, waits for the player, deduplicates loads with `sourceKey`, and calls `useAsset().loadSource(...)`. |
...
import type { PlayerSource, TAsset, UseAssetOptions } from "@/hooks/limeplay/use-asset"
...
interface CustomVideoAsset extends TAsset {
  title?: string
}
