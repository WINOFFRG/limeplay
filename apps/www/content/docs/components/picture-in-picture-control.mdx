...
<Callout>
  Requires [`pictureInPictureFeature`](/docs/hooks/use-picture-in-picture)
  registered in your [`createMediaKit`](/docs/components/media-provider).
</Callout>
  ...
  return (
    <PictureInPictureControl asChild shortcut="P">
      <Button variant="ghost" size="icon">
        <PictureInPictureIcon weight={active ? "fill" : "regular"} size={18} />
      </Button>
...
<Callout title="Browser Support" type="warning">
  PiP API is not natively supported in Firefox.
  [Reference](https://developer.mozilla.org/en-US/docs/Web/API/Picture-in-Picture_API)
</Callout>
