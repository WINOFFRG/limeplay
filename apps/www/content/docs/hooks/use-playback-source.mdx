...
```tsx
import type { TAsset } from "@/hooks/limeplay/use-asset"
import { PlaybackSourceController } from "@/hooks/limeplay/use-playback-source"
...
export function Source<TItem extends TAsset>(
...
| `sourceKey` | `PlaybackSourceControllerProps` | Optional stable key used to avoid duplicate playlist loads.                        |
...
| ----------- | ---------------------------- | ---------------------------------------------------------------------------------- |
| Load source | `PlaybackSourceController`   | Mounts a controller that normalizes source options into a load session.            |
...
| None             | `PlaybackSourceController` | The controller itself does not emit custom events.                                      |

## API Reference

<AutoTypeTable
  path="./registry/default/hooks/use-playback-source.ts"
  name="PlaybackSourceControllerProps"
/>
